...
fn render_node_details_card(active_id: Option<&str>, nodes: &[Node]) -> impl IntoElement {
    ...
    glass_card().w(px(320.0)).child(
        div()
            .flex_col()
            .gap_2()
            .child(div().text_sm().font_weight(FontWeight::BOLD).child(format!(
                "节点详情 ({})",
                node.map(|n| n.name.as_str()).unwrap_or("未选择")
            )))
            .child(detail_row(
                "地址",
                node.map(|n| n.details.address.as_str()).unwrap_or("-"),
            ))
            .child(detail_row(
                "协议",
                node.map(|n| n.protocol.as_str()).unwrap_or("-"),
            ))
            .child(detail_row(
                "加密",
                node.map(|n| n.details.encryption.as_str()).unwrap_or("-"),
            ))
            .child(detail_row(
                "UDP",
                if node.map(|n| n.details.udp).unwrap_or(false) {
                    "已启用"
                } else {
                    "否"
                },
            ...
            .child(detail_row(
                "TLS",
                if node.map(|n| n.details.tls).unwrap_or(false) {
                    "是"
                } else {
                    "否"
                },
            ...
            .child(detail_row(
                "传输",
                node.map(|n| n.details.transport.as_str()).unwrap_or("-"),
            ))
            .child(detail_row(
                "上次测试",
                node.map(|n| n.details.last_test.as_str()).unwrap_or("-"),
            )),
    )
}
